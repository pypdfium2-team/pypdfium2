    ...
    def __enter__(self) -> pdfium.FPDF_DOCUMENT:
        ...
        if page_count < 1:
            ...
            if last_error == pdfium.FPDF_ERR_SUCCESS:
                raise LoadPdfError(
                    "Even though no errors were reported, page count is invalid."
                )
            elif last_error == pdfium.FPDF_ERR_UNKNOWN:
                raise LoadPdfError(
                    "An unknown error occurred whilst attempting to load the document."
                )
    ...
    *,
    ...
    if not 0 <= page_index < page_count:
        raise PageIndexError(
            f"Page index {page_index} is out of bounds for document with {page_count} pages."
        )
