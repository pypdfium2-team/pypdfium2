...
class PdfDocument:
    ...
    def save(self, buffer, version=None):
        ...
        if not success:
            handle_error()
            raise RuntimeError("Saving the PDF failed, though no error detected.")
    ...
    def _render_base(
        ...
        Yields:
            Tuple of result object and page index.
        """
        ...
        with ProcessPoolExecutor(n_processes) as pool:
            for image, index in pool.map(invoke_renderer, page_indices):
                yield image, index
