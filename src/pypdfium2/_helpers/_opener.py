...
from pypdfium2._helpers.misc import handle_error
...
def open_pdf(input_data, password=None, autoclose=False):
    ...
    # Check that the document has been loaded correctly
    if pdfium.FPDF_GetPageCount(pdf) < 1:
        handle_error()
        raise RuntimeError("Invalid page count, though no error detected.")
