...
Parameters:
    ...
    origin (str):
        ...
        - ``system``, ``system/$BUILDER``, or ``system/$ENV/$BUILDER``: Loaded from a system location (i.e. not bundled with pypdfium2). The library will be searched for in directories given at setup time, and/or with :func:`ctypes.util.find_library()`. Note, this specifier may optionally include the binary builder, e.g. ``system/pdfium-binaries`` if the distributor is re-packaging the pdfium-binaries. Otherwise, it could be the name of the distribution, e.g. (hypothetically) ``system/debian``. If the package belongs to a specific environment, this extra information may be included before the builder, e.g. ``system/conda/pdfium-binaries``. To match against a possible builder, :meth:`~str.endswith` may be used.
    flags (tuple[str]):
