...
def main():
    ...
    _run_process(["docker", "run", "--security-opt", "label=disable", "--network", "host", "-e", "PREPARE_CMD", "-i", "--rm", "-v", f"{ProjectDir}:/pypdfium2", *docker_flags, container, shell, "/pypdfium2/utils/inside_docker.sh", str(args.wheel_path)], cwd=ProjectDir, env=env, check=True)
