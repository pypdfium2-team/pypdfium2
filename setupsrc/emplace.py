...
def _get_pdfium_with_cache(pl_name, req_ver, req_flags):
    ...
    windows_members = bool(int( os.environ.get("WITH_WINDOWS_MEMBERS", 0) ))
    build_pdfium_bindings(req_ver, flags=req_flags, ct_paths=ct_paths, windows=windows_members)
...
def stage_platfiles(pl_name, sub_target, pdfium_ver, flags, default_build_params=""):
    ...
    if pl_name == ExtPlats.system:
        ...
        if sub_target == "search":
            ...
            build_pdfium_bindings(pdfium_ver, flags=flags, guard_symbols=True, windows=True, rt_paths=())
