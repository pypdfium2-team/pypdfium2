...
# To bump these versions, first test locally and update any patches as needed. Then, make a branch and run "Sourcebuild" and "Sourcebuild Native" on CI to see if all targets continue to work. Commit the new version to the main branch only when all is green. Better stay on an older version for a while than break a target.
# Updating and testing the patch sets can be a lot of work, so we might not want to do this too frequrently.
SBUILD_NATIVE_PIN = 7827  # TODO update to 11b7e08 or above once we have a tag
