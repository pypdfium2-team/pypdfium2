...
- This release fixes compatibility with Python 3.7 and 3.6.
  ...
  * Also, fixed setup (i.e. source installation) with Python 3.6 and its max available setup dependency versions (that is, `setuptools` 59).
    ...
    - Note that ctypesgen continues to require Python `>=3.8` at this time (with 3.6 compatibility not being a priority for ctypesgen), but you can install with `--no-build-isolation` and let the reference bindings be used, or try adding ctypesgen's `src/` to `PYTHONPATH` to bypass setup, and see how it goes.
...
- `gn-dist` improvements: changed versioning and release process so that CI no longer needs to push to the repository. More documentation. manylinux2014 compatibility and build instructions.
