...
- In the CLI dispatcher, try to load only those modules that are actually used.
- CLI / `setup_logging()`: Fix warnings to be shown by removing an incorrect `logging.captureWarnings(True)` (that would require configuring the `py.warnings` logger or root logger which we don't do). Instead, use just `warnings.simplefilter("always")` for now.
  Also, fix `DEBUG_SYSFONTS` being a boolean option.
