...
*Setup*
- When pdfium binaries are downloaded implicitly on setup or `emplace.py` is run, we now use the version included with the last pypdfium2 release by default. This is so upstream API changes will not break pypdfium2 installation. `update.py` continues to default to the latest pdfium-binaries version.
