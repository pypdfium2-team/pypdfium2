...
- Fixed compatibility with Python 3.7 and 3.6.
  ...
  * To be clear though, **we do not plan to (and practically cannot) keep up compatibility with outdated Python versions indefinitely**. In fact, it is anticipated that setup compatibility may be given up some time soon in favor of contemporary Python packaging concepts, like migrating as much as possible to `pyproject.toml`. However, we are happy to restore compatibility *at this point*, and fix any *unintentional* breakage.
...
- TODO document CI changes (package installation cooldowns), MANIFEST.in fix, PEP 735 dependency groups etc., experimental pyemscrpiten builds
