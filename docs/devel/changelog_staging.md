...
- `pdfium-binaries` now use GitHub's new "immutable releases" feature on behalf of supply chain safety. Shortly before this feature was announced, pdfium-binaries had already introduced SLSA provenance, but this may soon be superseded by GitHub's own attestations / build provenance. Many thanks to Benoit Blanchon for all the work and co-operation. Also thanks to GitHub for adding immutable releases.
- `update.py`: Added `--verify` option to confirm authenticity of pdfium-binaries via digital attestations. Currently uses `slsa-verifier`, but may switch to `gh attestation verify` in the future. This is enabled in our release workflow, and will be automatically enabled for setup callers if the verification program is installed.
