...
- Changed output string handling once more, as even slicing the buffer directly still implies a copy. So let's use `memoryview` and `codecs.decode()` instead. We now create buffers of the expected type directly to avoid casts, and convert number of bytes to units via ceil division where needed.
