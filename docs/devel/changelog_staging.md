...
- This release fixes compatibility with Python 3.7 and 3.6.
  * Runtime support was inadvertently broken due to a faulty cached_property backport which held only one cache per class, not per instance as should have been.
  ...
  * This release replaces both `functools.cached_property` and the faulty `functools.lru_cache()` based backport with our own, backward compatible `cached_property` implementation along with thorough documentation and references to the author's gists.
