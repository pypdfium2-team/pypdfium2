...
- We now verify attestations of the `static-clang-images`. This took some refactoring the install script, so that verification can be performed on the host runner rather than inside a container. Thanks to Matthieu Darbois for adding attestations in the first place.
